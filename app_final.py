...
DATA_PATH = BASE_PATH.joinpath("archive").resolve()

# importing data
data = pd.read_csv(DATA_PATH.joinpath("players_21.csv.gz"))
...
players_options_over_25 = [
    {"label": df1["long_name"][i], "value": df1["long_name"][i]} for i in df1.index
]
players_options_under_25 = [
    {"label": df2["long_name"][i], "value": df2["long_name"][i]} for i in df2.index
]
...
# Updated: Replace dbc.FormGroup with simple containers like dbc.Row or similar
controls_player_1 = dbc.Card(
    [
        dbc.Row(
            [
                dbc.Label("Choose an old-school Player:"),
                html.Br(),
...
controls_player_2 = dbc.Card(
    [
        dbc.Row(
            [
                dbc.Label("Choose a new generation Player:"),
                html.Br(),
...
controls = dbc.Card(
    [
        dbc.Row([dbc.Label("Choose an Attribute:"), html.Br(), metric1_dropdown]),
        dbc.Row([dbc.Label("Choose an Attribute:"), html.Br(), metric2_dropdown]),
        dbc.Row([dbc.Label("Choose an Attribute:"), html.Br(), metric3_dropdown]),
    ],
    body=True,
    className="controls",
)
...
controls_club = dbc.Card(
    [
        dbc.Row([dbc.Label("Choose a League:"), html.Br(), metric_club_dropdown1]),
        dbc.Row([dbc.Label("Choose an attribute for x:"), html.Br(), metric_scatter_dropdown1]),
        dbc.Row([dbc.Label("Choose an attribute for y:"), html.Br(), metric_scatter_dropdown2]),
    ],
    body=True,
    className="controls",
)

# REPLACEMENT for cards_1, cards_2, cards_3, cards_4
                ...
                [
                    ...
                    dbc.Row(
                        [
                            ...
                            dbc.Col(dcc.Graph(id="league-graph1"), sm=3),
                            dbc.Col(dcc.Graph(id="league-graph2"), sm=3),
                            dbc.Col(dcc.Graph(id="league-graph3"), sm=3),
                        ],
                        align="center",
                    ),
                    dbc.Row(
                ...
                [
                    ...
                    dbc.Row(
                        [
                            ...
                            dbc.Col(dcc.Graph(id="club-graph1"), sm=5),
                            dbc.Col(dcc.Graph(id="club-graph2"), sm=5),
                        ],
                        align="center",
                    ),
                ]
    ...
    [
        html.A(
            dbc.Row(
                [
                    dbc.Col(
                        html.Img(src=app.get_asset_url("logo_white3.png"), height="130px"),
                        width=3,
                    ),
                    dbc.Col(
                        [
                            ...
                            html.Br(),
                            html.Label(
                                "Dashboard created by: Arshiyan , Alizeyy , Umama , Zaid",
                                className="label2",
...
@app.callback(
    [
        Output("league-graph1", "figure"),
        Output("league-graph2", "figure"),
        Output("league-graph3", "figure"),
    ],
    [
        Input("drop1", "value"),
        Input("drop2", "value"),
...
@app.callback(
    [Output("club-graph1", "figure"), Output("club-graph2", "figure")],
    [
...
def plots_clubs(league, x_val, y_val):
    ...
    plot_df2 = pd.pivot_table(
        plot_df2, values="short_name", index=["club_name"], columns=["isOver25"], aggfunc=np.sum
    ).reset_index()

    x = plot_df2["club_name"]
    # ensure both True & False columns exist
    if True not in plot_df2.columns:
        plot_df2[True] = 0
    if False not in plot_df2.columns:
        plot_df2[False] = 0

    y = plot_df2.iloc[:, -2:].div(plot_df2.iloc[:, -2:].sum(axis=1), axis=0)
